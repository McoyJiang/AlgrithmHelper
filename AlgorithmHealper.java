...
public class AlgorithmHealper {
    ...
    /**
     * 题目：输入一个由数字组成的字符串，请将它转化成整数
     * 例如：输入字符串"123"， 则输出整数类型123
     * 思路： 以"123"为例
     * 1 当扫描到字符串的第一个字符'1'时，因为是第一位所以得到整数1
     * 2 当扫描到第二个字符'2'时，因为之前已有一个1(相当于10)，所以得到整数位1*10 + 2
     * 3 当扫描到第三个字符'3'时，因为'3'的前面已经有了12(相当于120),所以得到12 * 10 + 2
     *
